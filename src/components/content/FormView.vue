<template>
  <el-form :model="comFormData" :ref="formOptions.baseOptions.ref" status-icon :rules="formOptions.baseOptions.validateRules">
    <!-- 输入框组件 -->
    <el-form-item
      ...
      <span class="detail-span" v-if="!isEdit">{{comFormData[formOption.prop]}}</span>
      <!-- input -->
      <el-input
        v-if="formOption.type === 'input' && isEdit"
        :value="comFormData[formOption.prop]"
        auto-complete="off"
...
export default {
  ...
  methods: {
    dialogCancel () {
      console.log(this.formOptions.baseOptions.ref)
